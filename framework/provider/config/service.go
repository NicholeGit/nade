...
// NewNadeConfig 初始化Config方法
func NewNadeConfig(params ...interface{}) (interface{}, error) {
...
// replace 表示使用环境变量 maps 替换 content 中的 env(xxx) 的环境变量
...
// 读取某个配置文件
func (conf *NadeConfig) loadConfigFile(folder string, file string) error {
	conf.lock.Lock()
...
func searchMap(source map[string]interface{}, path []string) interface{} {
	...
	if ok {
		...
		case map[interface{}]interface{}:
			...
			return searchMap(cast.ToStringMap(tmp), path[1:])
...
// 通过path来获取某个配置项
func (conf *NadeConfig) find(key string) interface{} {
	conf.lock.RLock()
